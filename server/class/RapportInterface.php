    ...
    function getForStaff() {
        try {
            ...
            $params->sql = "SELECT DATE_FORMAT(a.`start`, '%Y-%m-%d') as 'startdate', DATE_FORMAT(a.`start`, '%Y-%m-%d') as 'enddate', WEEK(a.`start`) as 'week', MONTH(a.`start`) as 'month',
                ...
                f.`code` as 'deliverytype', IF(e.`code`='default','REM',IF(e.`code`='office','REM','ONSITE')) as 'Location', 'EMEA' as 'Region', b.`salesForce` as 'deliverable', a.`googleICalUID` as 'EventID', 'na' as 'ConsultingID', CONCAT(a.`title`,', comment:',a.`cmt`) as 'Notes'
